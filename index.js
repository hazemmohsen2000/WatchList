    ...
    searchButton.addEventListener("click", function(e) {
        ...
        fetch(`https://www.omdbapi.com/?s=${searchInput.value}&type=movie&apikey=cf0fdf8`)
            ...
            .then(data => {
                ...
                searchList.forEach(movieTitle => {
                    fetch(`https://www.omdbapi.com/?t=${movieTitle}&apikey=cf0fdf8`)
                        .then(res => res.json())
                        .then(data => {
                            console.log(data);
                            summaryHTML += `
...
if (homeButton) {
    ...
    while (i--) {
        // Parse the stored JSON data to objects
        let movieData = JSON.parse(localStorage.getItem(keys[i]))
        if (movieData && typeof movieData === 'object') {
            watchlistPicks.push(movieData)
        } else {
            console.error('Invalid data found in local storage:', movieData)
        }
    }
    console.log(watchlistPicks)

    // Display watchlist items if available
